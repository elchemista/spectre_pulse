defmodule Spectre.Pulse.Route do
  ...
  def new(%__MODULE__{} = route), do: route |> Map.from_struct() |> new()

  def new(attrs) when is_list(attrs) do
    if Keyword.keyword?(attrs),
      do: attrs |> Map.new() |> new(),
      else: {:error, Error.not_sent(:routing, {:invalid_route, attrs})}
  end
  ...
  defp validate(%__MODULE__{} = route) do
    with {:ok, address} <- Address.normalize(route.address) do
      cond do
        is_nil(route.id) ->
